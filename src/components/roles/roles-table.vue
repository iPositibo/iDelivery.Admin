<template>
    <div>
        ...
        <v-data-table :headers="tableHeaders" :items="roleTable" sort-by="roleId" class="elevation-1">
            <template v-slot:top>
                                    ...
                                    <v-form ref="form" v-model="isValid" :lazy-validation="lazy">
                                        <v-row>
                                            <v-col cols="12" sm="6" md="6">
                                                <v-text-field v-model="editedItem.roleName" type="text"
                                                    :rules="rolenameRules" label="Role name" required></v-text-field>
                                            </v-col>
                                        </v-row>
                                    </v-form>
                                </v-container>
    ...
    export default {
        data: () => ({
            ...
            rolenameRules: [
                ...
                v => (v && v.length <= 40) || 'Role Name must be less than 40 characters',
            ],
            interval: {},
            value: 0,
            loading: false
        ...
        methods: {
            editItem(item) {
                this.editedIndex = this.roleTable.indexOf(item)
                this.editedItem = Object.assign({}, item)
                this.dialog = true
            },
            deleteItem(item) {
                const index = this.roleTable.indexOf(item)
                const isConfirmed = confirm('Are you sure you want to delete this item?') 

                if(isConfirmed){
                    const self = this
                    RoleService.deleteRole(item.roleId)
                        .then(() => {
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "Role has been deleted successfully."
                            self.roleTable.splice(index, 1)
                        })
            ...
            save() {
                ...
                if (this.editedIndex > -1) {
                    RoleService.editRole(self.editedItem.roleId, self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "Role has been updated successfully."
                        })
                        .catch(function (error) {
                            self.loading = false
                            self.alertInfo = {
                    ...
                    Object.assign(self.roleTable[self.editedIndex], self.editedItem)

                } else {
                    RoleService.addRole(self.editedItem)
                        .then(() => {
                            self.loading = false
