<template>
    <div>
        ...
        <v-data-table :headers="tableHeaders" :items="FAQTable" sort-by="faqid"
            class="elevation-1">
            <template v-slot:top>
                                    ...
                                    <v-form ref="form" v-model="isValid" :lazy-validation="lazy">
                                        <v-row>
                                            <v-col cols="12" sm="6" md="12">
                                                <v-textarea single-line auto-grow rows="1" v-model="editedItem.faqcontent" type="text" label="FAQ" required>
                                                </v-textarea>
                                            </v-col>
                                        </v-row>
                                    </v-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            editItem(item) {
                this.editedIndex = this.FAQTable.indexOf(item)
                this.editedItem = Object.assign({}, item)
                this.dialog = true
            },
            deleteItem(item) {
                const index = this.FAQTable.indexOf(item)
                const isConfirmed = confirm('Are you sure you want to delete this item?')

                if(isConfirmed){
                    const self = this
                    FAQService.deleteFAQ(item.faqid)
                        .then(() => {
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "FAQ has been deleted successfully."
                            self.FAQTable.splice(index, 1)
                        })
            ...
            save() {
                if (this.editedIndex > -1) {
                    let self = this
                    self.loading = true
                    FAQService.editFAQ(self.editedItem.faqid, self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "FAQ has been updated successfully."
                        })
                        .catch(function (error) {
                            self.loading = false
                            self.alertInfo = {
                    ...
                    Object.assign(self.FAQTable[self.editedIndex], self.editedItem)

                } else {
                    let self = this
                    FAQService.addFAQ(self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successMessage = "FAQ has been saved successfully."

                            FAQService.getAllFAQs()
                                .then(response => {
                                    self.FAQTable = response.data
                                    self.loading = false
