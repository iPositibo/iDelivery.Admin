<template>
    <div>
        ...
        <v-data-table :headers="tableHeaders" :items="riderStatusTable" sort-by="riderStatusId" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat color="white">
                    <v-toolbar-title>Rider Status</v-toolbar-title>
                                ...
                                <v-container>
                                    ...
                                    <v-row>
                                        <v-col cols="12" sm="6" md="6">
                                            <v-textarea single-line auto-grow rows="1" v-model="editedItem.status" type="text" :rules="riderStatusRules"
...
<script>
    import RiderStatusService from './rider-status.service'
    ...
    export default {
        data: () => ({
            ...
            lazy: false,
            riderStatusRules: [
                v => !!v || 'Rider Status is required',
                v => (v && v.length <= 40) || 'Rider Status must be less than 40 characters',
            ],
            interval: {},
            value: 0,
            loading: false
        ...
        props: {
            riderStatusTable: null,
            tableHeaders: null
        },
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'New Rider Status' : 'Edit Rider Status'
            },
        },
        watch: {
            dialog(val) {
        ...
        methods: {
            editItem(item) {
                this.editedIndex = this.riderStatusTable.indexOf(item)
                this.editedItem = Object.assign({}, item)
                this.dialog = true
            },
            deleteItem(item) {
                const index = this.riderStatusTable.indexOf(item)
                const isConfirmed = confirm('Are you sure you want to delete this item?')

                if(isConfirmed){
                    const self = this
                    RiderStatusService.deleteRiderStatus(item.riderStatusId)
                        .then(() => {
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "Status has been deleted successfully."
                            self.riderStatusTable.splice(index, 1)
                        })
            ...
            save() {
                ...
                if (this.editedIndex > -1) {
                    RiderStatusService.editRiderStatus(self.editedItem.riderStatusId, self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successDialog = true
                            self.errorDialog = false
                    ...
                    Object.assign(self.riderStatusTable[self.editedIndex], self.editedItem)

                } else {
                    RiderStatusService.addRiderStatus(self.editedItem)
                        .then(() => {
                            self.loading = false
                            ...
                            RiderStatusService.getAllRiderStatus()
                                .then(response => {
                                    self.riderStatusTable = response.data
                                    self.loading = false
                                    self.successDialog = true
                                    self.errorDialog = false
                                })
