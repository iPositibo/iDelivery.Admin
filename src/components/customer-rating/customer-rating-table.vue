<template>
    <div>
        ...
        <v-data-table :headers="tableHeaders" :items="customerRatingTable" sort-by="customerRatingId" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat color="white">
                    <v-toolbar-title>Customer Rating</v-toolbar-title>
                                ...
                                <v-container>
                                    ...
                                    <v-row>
                                        <v-col cols="12" sm="6" md="6">
                                            <v-select item-text="firstName" item-value="customerId" :items="customers"
                                                v-model="editedItem.customerId" :rules="[v => !!v || 'Customer is required']" label="Customer" required>
                                            </v-select>
                                        </v-col>
...
<script>
    ...
    export default {
        ...
        props: {
            customerRatingTable: null,
            tableHeaders: null
        },
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'New Customer rating' : 'Edit Customer rating'
            },
        },
        watch: {
            dialog(val) {
        ...
        methods: {
            editItem(item) {
                this.editedIndex = this.customerRatingTable.indexOf(item)
                this.editedItem = Object.assign({}, item)
                this.dialog = true
            },
            deleteItem(item) {
                const index = this.customerRatingTable.indexOf(item)
                const isConfirmed = confirm('Are you sure you want to delete this item?')
 
                if(isConfirmed){
                    const self = this
                    CustomerRatingService.deleteCustomerRating(item.customerRatingId)
                        .then(() => {
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "Customer Rating has been deleted successfully."
                            self.customerRatingTable.splice(index, 1)
                        })
            ...
            save() {
                ...
                if (self.editedIndex > -1) {
                    CustomerRatingService.editCustomerRating(self.editedItem.customerRatingId, self.editedItem)
                        .then(() => {
                            self.loading = false

                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "Customer Rating has been updated successfully."
                        })
                        .catch(function (error) {
                            self.loading = false
                            self.alertInfo = {success: false, error }
                    ...
                    Object.assign(self.customerRatingTable[self.editedIndex], self.editedItem)

                } else {
                    CustomerRatingService.addCustomerRating(self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successMessage = "Status has been saved successfully."

                            CustomerRatingService.getAllCustomerRatings()
                                .then(response => {
                                    self.customerRatingTable = response.data
                                    self.loading = false
