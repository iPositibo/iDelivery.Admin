<template>
    <div>
        ...
        <v-data-table :headers="tableHeaders" :items="AppRatingTable" sort-by="appRatingId" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat color="white">
                    <v-toolbar-title>App Rating</v-toolbar-title>
                                    ...
                                    <v-form ref="form" v-model="isValid" :lazy-validation="lazy">
                                        <v-row>
                                            ...
                                            <v-col cols="12" sm="6" md="8">
                                                <v-textarea single-line auto-grow rows="1" v-model="editedItem.feedback"
                                                    type="text" label="Feedback" required>
                                                </v-textarea>
                                            </v-col>
                                            <v-col cols="12" sm="6" md="6">
                                                <v-rating v-model="editedItem.rating" type="number" label="Star"
                                                    color="yellow darken-3" background-color="grey darken-1"
                                                    empty-icon="$vuetify.icons.ratingFull" hover :rules="['Required']">
                                                </v-rating>
                                            </v-col>
                                        </v-row>
                                    </v-form>
...
<script>
    ...
    export default {
        ...
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'New App rating' : 'Edit App rating'
            },
        },
        watch: {
            dialog(val) {
        ...
        methods: {
            editItem(item) {
                this.editedIndex = this.AppRatingTable.indexOf(item)
                this.editedItem = Object.assign({}, item)
                this.dialog = true
            },
            deleteItem(item) {
                const index = this.AppRatingTable.indexOf(item)
                const isConfirmed = confirm('Are you sure you want to delete this item?')

                if (isConfirmed) {
                    const self = this
                    AppRatingService.deleteAppRating(item.appRatingId)
                        .then(() => {
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "App rating has been deleted successfully."
                            self.AppRatingTable.splice(index, 1)
                        })
            ...
            save() {
                if (this.editedIndex > -1) {
                    ...
                    AppRatingService.editAppRating(self.editedItem.appRatingId, self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successDialog = true
                            self.errorDialog = false
                            self.successMessage = "App rating has been updated successfully."
                        })
                        .catch(function (error) {
                            self.loading = false
                            self.alertInfo = {
                    ...
                    Object.assign(self.AppRatingTable[self.editedIndex], self.editedItem)

                } else {
                    let self = this
                    AppRatingService.addAppRating(self.editedItem)
                        .then(() => {
                            self.loading = false
                            self.successMessage = "App rating has been saved successfully."

                            AppRatingService.getAllAppRatings()
                                .then(response => {
                                    self.AppRatingTable = response.data
                                    self.loading = false
